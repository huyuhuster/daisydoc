...
:::{note} 若实验过程中未采集背景或暗场数据，可不导入，直接跳过。 :::
...
:::{note} 该区域选择不能超过300*300，若样品占满整个区域，这步可跳过。 :::
...
:::{note} 由于选区的大小决定了后续计算的占用内存，因此需要选区还需考虑计算机本身性能。若超过内存，则后续无法计算。 :::
